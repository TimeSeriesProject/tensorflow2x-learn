...
for epoch in range(epochs):
    ...
    for step, (batch_x, batch_y) in enumerate(train_db):
        ...
        # 修正一阶动量
        m_w_correction = m_w/(1-tf.pow(beta, int(global_step)))
        m_b_correction = m_b / (1 - tf.pow(beta, int(global_step)))

        # 修正二阶动量
        v_w_correcttion = v_w/(1 - tf.pow(beta2, int(global_step)))
        v_b_correction = v_b/(1 - tf.pow(beta2, int(global_step)))

        # 更新
        w.assign_sub(lr * m_w_correction/tf.sqrt(v_w_correcttion))
